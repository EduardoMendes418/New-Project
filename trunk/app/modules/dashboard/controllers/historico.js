		        ...
		        headers: {'Content-Type': 'application/x-www-form-urlencoded'}
                    ...
                    if(response[0].retorno == 1){
                       ...
                       $scope.lista = response;                      
                        var tmp = response.filter(function(response){
                            $scope.total++;
                            $scope.valorTotal += parseFloat(response.valorReal);
                            $scope.distTotal += parseFloat(response.distanciaReal);
                        });
                    }else{
