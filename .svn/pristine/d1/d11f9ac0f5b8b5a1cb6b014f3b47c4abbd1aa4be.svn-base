...
angular.module('googleAutocomplete.places', ['googleAutocomplete.places.directives']).run(['$window', function($window){
        // Check for dependencies
        if(
            angular.isUndefined($window.google) ||
            angular.isUndefined($window.google.maps) ||
    ...
    .directive('gmPlacesAutocomplete', ['$rootScope', 'googleAutocomplete.config', 'logger', function($rootScope, gmConfig, logger){
        ...
        var configOptions = (gmConfig.places && gmConfig.places.autocomplete) || {};
         ...
         return {
                  ...
                  var cityBounds =  new google.maps.LatLng(-22.9099,-47.0626);

                    this._options = angular.extend({location: cityBounds
                                                    ,radius: 150000 // em metros, 150 km
                                                    ,types: ['address']
                                                    ,componentRestrictions: {country: 'br'}}                    
                                                    , configOptions
                                                    , $scope.$eval($attrs.gmOptions));
